...
{   //Déclaration des instances du projet
    ...
    Application* app(new Application()); //Création d'une instance de la classe Application héritant de wxApp
    wxApp::SetInstance(app); 
    wxEntryStart(argc, argv); //Initialisation de wxWidgets
    Control* control(new Control(wxT("Menu"),wxSize(640*0.5, 480*1.5)));	
    //(*app).setSysteme(&sys); 
   // ((wxTheApp->OnInit(); //Appel de la fonction d'initialisation personnalisée.
    //init(app);
